...
def calculate_reynolds_number(
    velocity: float, characteristic_length: float, fluid_density: float, fluid_viscosity: float
) -> float:
    """
...
def calculate_prandtl_number(fluid_viscosity: float, fluid_specific_heat: float, fluid_thermal_conductivity: float) -> float:
    """
...
def calculate_nusselt_number_external_flow(
    reynolds_number: float, prandtl_number: float, geometry: str = "flat_plate", strict: bool = True
) -> float:
    """
...
def calculate_radiation_heat_transfer(emissivity: float, area: float, temperature_1: float, temperature_2: float) -> float:
    """
...
def estimate_sky_temperature(
    ambient_temperature: float, dew_point: Optional[float] = None, cloud_cover: Optional[float] = None
) -> float:
    """
...
def calculate_lmtd(
    t_hot_in: float, t_hot_out: float, t_cold_in: float, t_cold_out: float, flow_arrangement: str = "counterflow"
) -> float:
    """
    ...
    try:
        return LMTD(Thi=t_hot_in, Tho=t_hot_out, Tci=t_cold_in, Tco=t_cold_out, counterflow=counterflow)
    except Exception as e:
        # Re-raise with more context
        raise ValueError(f"LMTD calculation failed: {e}. Check for temperature crossover.")


def calculate_ntu_effectiveness(ntu: float, capacity_ratio: float, flow_arrangement: str = "counterflow") -> float:
    """
    ...
    logger.debug(
        f"ht.effectiveness_from_NTU: NTU={ntu:.3f}, Cr={capacity_ratio:.3f}, " f"subtype={subtype} → ε={effectiveness:.4f}"
    )
