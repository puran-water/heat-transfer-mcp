...
class WeatherDataService:
    ...
    def get_design_conditions(
        ...
        try:
            ...
            if df is None or df.empty:
                return {"error": "No weather data available for this location", "location": {"lat": lat, "lon": lon}}
    ...
    def _process_percentiles(self, df: Any, percentiles: List[float], cache_key: str) -> Dict[str, Any]:  # pandas DataFrame
        ...
        if len(df) > 0:
            ...
            try:
                ...
                if resolution == "daily":
                    ...
                    if coverage < 80:
                        warnings.append(
                            f"Data coverage is {coverage:.0f}% (gaps detected). Meteostat station data may be incomplete."
                        )
                        if coverage < 50:
                            data_quality = "poor"
            except Exception:
