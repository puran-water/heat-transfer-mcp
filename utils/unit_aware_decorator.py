...
PARAMETER_TYPES = {
    ...
    'velocity': {
        'target_unit': 'm/s',
        'common_units': ['ft/s', 'fps', 'mph', 'm/s', 'km/hr'],
        'default_imperial': 'ft/s'
    },
    
    # Power/heat conversions
    'power': {
        'target_unit': 'watt',
        'common_units': ['BTU/hr', 'hp', 'HP', 'W', 'kW', 'MW'],
        'default_imperial': 'BTU/hour'
    },
        ...
        if key in mappings:
            ...
            if param_type == 'nested' and isinstance(value, dict):
                ...
                for nested_key, nested_val in value.items():
                    nested_converted[nested_key] = convert_value(
                        nested_val, 'length', f"{key}.{nested_key}"
                    )
            ...
            # Handle lists (like wall_layers)
                ...
                for item in value:
                    if isinstance(item, dict) and 'thickness' in item:
                        item_copy = item.copy()
                        item_copy['thickness'] = convert_value(
                            item['thickness'], 'length', f"{key}.thickness"
                        )
