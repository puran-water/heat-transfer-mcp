    ...
    try:
        ...
        for i, layer in enumerate(layers):
            ...
            if thickness is None or thickness <= 0:
                return json.dumps({
                    "error": f"Invalid thickness provided for layer {i+1}."
                })
            if k is None and material_name is None:
                return json.dumps({
                    "error": f"Layer {i+1} must have either 'thermal_conductivity_k' or 'material_name'."
                })
            ...
            if k <= 0:
                return json.dumps({
                    "error": f"Invalid thermal conductivity k={k} for layer {i+1}."
                })
            ...
            if geometry.lower() == 'flat_wall':
                ...
                if current_radius is None:
                    return json.dumps({
                        "error": "Internal error: current_radius not set for cylinder."
                    })
        ...
        if geometry.lower() == 'cylinder':
            ...
            if outer_diameter and not math.isclose(calculated_outer_diameter, outer_diameter, rel_tol=1e-6):
                ...
                logger.warning(f"Provided outer diameter {outer_diameter} does not match calculated {calculated_outer_diameter}")
        ...
        if geometry.lower() == 'flat_wall':
            ...
            return json.dumps({
                "error": f"Unsupported geometry: {geometry}. Use 'flat_wall' or 'cylinder'."
            })
