...
def calculate_solar_radiation_on_surface(
    ...
    try:
        ...
        if solar_altitude is None or solar_azimuth is None:
            ...
            sin_altitude = math.sin(lat_rad) * math.sin(declination) + math.cos(lat_rad) * math.cos(declination) * math.cos(
                hour_angle
            )
            ...
            # Solar azimuth angle
            cos_azimuth = (
                math.sin(declination) * math.cos(lat_rad) - math.cos(declination) * math.sin(lat_rad) * math.cos(hour_angle)
        ...
        if direct_normal_irradiance_dni is None or diffuse_horizontal_irradiance_dhi is None:
            ...
            if solar_altitude_deg > 0:
                air_mass = 1.0 / (
                    math.sin(math.radians(solar_altitude_deg)) + 0.50572 * (6.07995 + solar_altitude_deg) ** (-1.6364)
                )
            else:
                air_mass = 0
            ...
            beam_transmittance = 0.56 * (
                math.exp(-0.65 * air_mass * altitude_factor) + math.exp(-0.095 * air_mass * altitude_factor)
            )
        ...
        cos_aoi = math.sin(solar_altitude_rad) * math.cos(surface_tilt_rad) + math.cos(solar_altitude_rad) * math.sin(
            surface_tilt_rad
        ...
        result = {
            ...
            "surface_orientation": {"tilt_deg": surface_tilt, "azimuth_deg": surface_azimuth},
            "location": {"latitude_deg": latitude, "longitude_deg": longitude, "altitude_m": altitude},
