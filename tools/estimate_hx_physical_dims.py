...
def estimate_hx_physical_dims(
    ...
    try:
        ...
        if required_area <= 0 or tube_outer_diameter <= 0 or tube_length_effective <= 0:
            return json.dumps({
                "error": "Required area, tube outer diameter, and tube length must be positive."
            })
        ...
        if area_per_tube <= 0:
            return json.dumps({
                "error": "Calculated area per tube is zero or negative."
            })
        ...
        if method.upper() == "HEDH":
            ...
            D_bundle = tube_outer_diameter + (1.0/.78)**0.5 * tube_pitch * (C1*N_tubes)**0.5
            ...
            if number_of_tube_passes == 1:
                ...
                return json.dumps({
                    "error": "Unsupported number_of_tube_passes for VDI method. Use 1, 2, 4, 6, or 8."
                })
            ...
            return json.dumps({
                "error": "Invalid method. Choose 'HEDH' or 'VDI'."
            })
