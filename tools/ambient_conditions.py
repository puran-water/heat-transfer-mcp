...
def get_ambient_conditions(
    ...
    if not METEOSTAT_AVAILABLE:
        return json.dumps({
            "error": "Weather data retrieval is not available. The meteostat package is not installed."
        })
        
    if not PANDAS_AVAILABLE:
        return json.dumps({
            "error": "Weather data processing is not available. The pandas package is not installed."
        })
    ...
    try:
        ...
        try:
            ...
            if data.empty:
                return json.dumps({
                    "result": [],
                    "message": "No weather data found for the specified location/date range."
                })
